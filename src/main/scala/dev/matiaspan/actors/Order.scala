...
object Order {
  ...
  case class CreateSync(id: Int, items: Int, price: Float, userID: Int, replyTo: ActorRef[Response]) extends Command with CborSerializable
  ...
  def apply(id: String): Behavior[Order.Command] = Behaviors.setup { context => 
    ...
    Behaviors.receiveMessage { message =>
      message match {
        ...
        case CreateSync(id, items, price, userID, replyTo) =>
          if (state.id != 0) {
            println(s"[${cluster.selfMember.address}]  [$id] - Order order with ${items} items, price ${price} and userID ${userID} already exists")
